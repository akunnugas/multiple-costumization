...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $butirLK = IndikatorLaporanKinerja::where('id_pengisian_panduan', $pengisianPanduan->id)
            ->whereIn('nomor_indikator', ['1.1.a.1', '1.2.3', 'XREF.1', 'XREF.2', '1.1.d.13'])->get();

        $indikatorLK = array_combine($butirLK->pluck('nomor_indikator')->toArray(), $butirLK->pluck('id')->toArray());

        $penilaianPanduan = PenilaianPanduan::where(['kode_penilaian_panduan' => 'IAPS5.1-S1-Ung', 'apakah_data_default' => true, 'waktu_dihapus' => null])->first();
        $matriksPenilaian = PenilaianMatriks::where('id_penilaian_panduan', $penilaianPanduan->id)
            ->whereIn('nomor_penilaian', ['B106D', 'B108C', 'B110C', 'B112A', 'B115', 'B323A', 'B323B'])->get();
