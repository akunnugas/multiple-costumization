...
class JadwalAuditPolicy
{
    ...
    public function view(SiakadUser $user, JadwalAudit $jadwalAudit)
    {
        $userRole = $user->kode_role;

        if ($userRole === Role::ROLE_AUDITOR || $userRole === Role::ROLE_KAPRODI || $userRole === Role::ROLE_AUDITEE) {
            return $this->authorizeRole($user, $jadwalAudit);
        }

        $userUnit = session()->get('user.unit_kerja');
        if (!empty($userUnit)) {
            return $this->authorizeUnitKerja($userUnit, $jadwalAudit);
        }

        return false;
    }

    protected function authorizeRole(SiakadUser $user, JadwalAudit $jadwalAudit)
    {
        $userRole = $user->kode_role;
        ...
        $data = DB::selectOne($sql, [
            'id_audit_periode' => $jadwalAudit->id_audit_periode,
            'id_user' => $user->id,
    ...
    protected function authorizeUnitKerja(mixed $userUnit, JadwalAudit $jadwalAudit)
    {
        $userUnit = $jadwalAudit->organizations()
            ->where('info_left', '>=', $userUnit->info_left)
