                ...
                <div class="button-group">
                    <div class="button-group__left">
                        ...
                        @if ($canUpdate && !$disableEdit)
                            <a class="btn btn_primary btn_xs"
                                href="{{ Page::editURL($resourceId) }}?jenis_edisi={{ $rawData['type'] }}">
                                Set Mapping @if ($rawData['type'] == 'pr')
                                    Laporan Kinerja
                                @else
                    ...
                    <div class="button-group__mobile">
                        ...
                        @if ($canUpdate && !$disableEdit)
                            <a class="btn btn_primary btn_icon btn_xs"
                                href="{{ Page::editURL($resourceId) }}?jenis_edisi={{ $rawData['type'] }}">
                ...
                <div class="content content-full">
                        ...
                        @if ($isProdi)
                            <div class="grid">
                                ...
                                <div class="col-8 col-sm-8 col-md-8 col-lg-8">
                                    <span class="row-data__value" style="font-size: 0.75rem;">
                                        <span class="row-data__colon">:</span>
                                        {{ $rawData['detail']['pengisian_panduan_label'] ?? '-- Belum ada panduan yang dipilih --' }}
                                    </span>
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($isProdi)
                            <div class="grid">
                                ...
                                <div class="col-8 col-sm-8 col-md-8 col-lg-8">
                                    <span class="row-data__value" style="font-size: 0.75rem;">
                                        <span class="row-data__colon">:</span>
                                        <x-spmi::fields.mapping_status :value="!empty($rawData['detail']['pengisian_panduan_label'])" />
                                    </span>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="content__body">
                        ...
                        <div style="display: flex; gap: 0.5rem; max-width: {{ $width }};">
                            ...
                            @endif
                            @if (!empty($rawData['detail']['pengisian_panduan_options']))
                                <x-core::select label="Pilih Panduan Pengisian" :options="$rawData['detail']['pengisian_panduan_options']" :selected="$rawData['detail']['selected_pengisian_panduan']"
                            ...
                            <script>
                                ...
                                function setFilterPanduan(value) {
                                    const url = new URL(window.location.href);
                                    url.searchParams.set('id_pengisian_panduan', value);
                                    window.location.href = url.toString();
                                }
                            </script>
                        </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    @forelse ($rawData['indicator_list'] as $row)
                                        <tr>
                                            <td><span
                                                    style="margin-left: {{ $row['info_level'] * 18 }}px">{{ $row['nama_indikator_evaluasi_diri'] ?? $row['nama_indikator_laporan_kinerja'] }}</span>
                                            </td>
                                        ...
                                        @foreach ($row['children'] as $item)
                                            <tr>
                                                <td><span
                                                        style="margin-left: {{ $item['info_level'] * 18 }}px">{{ $item['nama_indikator_evaluasi_diri'] ?? $item['nama_indikator_laporan_kinerja'] }}</span>
                                                </td>
                                                <td><x-spmi::fields.apakah_iku_ikt :value="$item['apakah_data_default']" :type="'secondary'"
                                                        :decoration="true" /></td>
                                            </tr>
                                        @endforeach
                                    @empty
                                        <tr>
                                            <td colspan="2" style="text-align: center;">Belum ada data mapping
                                                indikator yang tersedia</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
