...
@php
    ...
    $periodId = $filter['mp.id_pengisian_panduan']['selected'];
            ...
            <x-core::table>
                        ...
                        <div class="col-md-2 col-sm-8">
                            <div class="box-table__wrapper" style="display: flex;">
                                @foreach ($filter as $key => $item)
                                    <x-core::select name="{{ $key }}" label="{{ $item['label'] }}"
                ...
                <div class="alert alert alert_helper">
                    <div class="alert__content">
                        <p>
                            Data berikut menampilkan daftar keterkaitan antara <b>panduan penilaian</b> dan <b>instrumen
                            akreditasi</b> yang digunakan dalam proses audit mutu internal. Pastikan setiap panduan <b>telah
                            terhubung dengan instrumen yang sesuai</b> sebelum digunakan dalam proses penilaian.
                        </p>
                    </div>
                                ...
                                <tr>
                                    @foreach ($header as $i => $item)
                                        @php
                                            ...
                                            if ($item['field'] == 'nama_indikator_laporan_kinerja') {
                                                $label = 'Nama Butir';
                                            }
                                        @endphp
                                        <th data-no="{{ $no }}">
                                ...
                                @foreach ($data as $key => $row)
                                    <tr>
                                        ...
                                        @foreach ($header as $item)
                                            ...
                                            <td>
                                                <p
                                                    @if ($item['field'] == 'nama_indikator_laporan_kinerja') style="margin-left: {{ $row->info_level * 18 }}px" @endif>
                                                    {{ $value }}</p>
                                            </td>
                                        @endforeach
                                        <td>
                                            <input @if (!(isset($edit) && empty($item['readonly']) && $canUpdate)) disabled @endif type="checkbox"
                                                name="data[{{ $row->id }}][selected]"
                                                @if ($row->is_mapping) checked @endif value="1" />
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
