    ...
    <x-core::quantum-3.layouts.main.container :$menu :$title :$subtitle class="row" :withHeader="false">
        ...
        <form wire:submit="save" class="qn-form needs-validation py-3 ps-3 position-relative col-xl-6 {{ $toc ? 'col-md-9 offset-xxl-1' : 'col-xl-6 offset-xl-3' }}" novalidate>
            <div class="row px-3 row-cols-1 gy-4">
                ...
                <x-core::quantum-3.layouts.html.alert class="m-0" :data="$alert" />
                ...
                @foreach ($data as $sectionId => $section)
                    ...
                    @if ($sectionId == 'informasi-indikator-sasaran')
                        <x-core::quantum-3.layouts.create.card :$sectionId {{ $attributes }} :$showCollapseInSection>
                            @foreach ($indikatorFields as $index => $indikator)
                                <div wire:key='indikator-{{ $index }}' class="col-md-12 ">
                                    @if (!$loop->first)
                                        <hr>
                                    @endif
                                    <div class="d-flex justify-content-between align-items-center pb-3">
                                        <span class="fw-semibold fs-5">Indikator {{ $loop->iteration }}</span>
                                        @if (!$loop->first)  
                                            <x-core::quantum-3.button variant="danger" leadingIcon="trash" wire:click='removeIndikator({{ $index }})'>Hapus</x-core::quantum-3.button>
                                        @endif
                                    </div>
                            ...
                            <div class="col-md-12 mt-3 d-flex justify-content-center">
                                <x-core::quantum-3.button variant="light" leadingIcon="plus" wire:click='addIndikator()'>
                                    Tambah Indikator
