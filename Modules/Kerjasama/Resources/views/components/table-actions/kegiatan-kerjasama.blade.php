...
<div class="text-center d-flex gap-1 justify-content-center">
    @if ($showDetail)
        <x-core::quantum-3.button leading-icon="eye-solid" variant="outline-secondary" icon="true" size="sm" :href="route('kerjasama.kegiatan.show', ['kegiatan' => $data['id']]) . '?backUrl=' . url()->current()" />
    @endif
    @if ($canUpdate)
        @php
            $attributes = [];
