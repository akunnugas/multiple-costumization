...
<x-core::quantum-3.layouts.main :$menu :$title :$subtitle :withContainer="true">
    ...
    <x-slot:action>
        @if (!empty($customAction))
            {{ $customAction }}
        @else
            <x-core::quantum-3.layouts.list.action :$isReference :$canCreate :$canDelete :$withSync :$withExport :$withImport :$showDeleteChecked 
                :createLabel="$createLabel" :$customCreateLink :syncLabel="$syncLabel" />
        @endif
    </x-slot:action>
            ...
            @if ($slot->isEmpty())
                <x-core::quantum-3.table :$navTab>
                    ...
                    @if (!$create && empty($data->items))
                        ...
                        @endphp
                        <x-core::quantum-3.handler :$customCreateLink :$isReference :$canCreate :$title :$subtitle :$createLabel />
                    @endif
