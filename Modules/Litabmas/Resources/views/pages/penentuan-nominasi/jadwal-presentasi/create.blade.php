...
@php
    ...
    // parameter form
    $method = 'POST';
    $action = route('litabmas.penentuan-nominasi.jadwal-presentasi', $idPengajuanPendanaan);
    if (!empty($resourceId)) {
        $method = 'PUT';
        $action .= '/' . $resourceId;
    }
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const handleCheckTipeKegiatan = () => {
                    ...
                    // if tipeKegiatanGroup is null
                    if (!tipeKegiatanGroup) {
                        //check value default dari radio
                        const tipeKegiatanGroupDefault = document.querySelector('input[name="tipe_kegiatan"][value="{{ PengajuanPendanaanJadwalPresentasi::TIPE_KEGIATAN_OFFLINE }}"]');
                        tipeKegiatanGroupDefault.checked = true;
