...
@if(!empty($value))
    @php
        $text = PengajuanPendanaanStatus2::getLabelStatus($value);
        $colorVariant = PengajuanPendanaanStatus2::getStatusColorVariant($value);
        ...
        $proposal = PengajuanPendanaan::find($data['id']);
        $listAgenda = $service->getTimelineStatusAgendaKegiatanByIdPengajuanPendanaan($data['id']);

        if ($kodeAgenda && $isDosen) {
            $agendaPengumuman = array_filter($listAgenda, function ($item) use ($kodeAgenda){
                return $item->kode_agenda === $kodeAgenda;
            });
            $agendaPengumuman = array_shift($agendaPengumuman);

            $isProses = false;
            if ($value === PengajuanPendanaanStatus2::LEVEL5_LOLOS_ADMINISTRASI) {
        ...
        $jumlahAgenda = count($listAgenda);
        $jumlahAgendaSelesai = 0;
    ...
    <small style="margin-left: 10px;">{{ $jumlahAgendaSelesai }} dari {{ $jumlahAgenda }} tahapan kegiatan</small>
@endif
