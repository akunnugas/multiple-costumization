@php
    $listBertugas = explode(',', $data['bertugas_sebagai']);
    ...
    foreach ($listBertugas as $cVal) {
        ...
        $cVal = explode(')', $cVal[1]);
        $cVal = $cVal[0];
        $agenda[trim($realVal)] = explode(' - ', $cVal);
    }
    ...
    $mapped = [
        'Review Proposal' => 'status_penilaian_proposal',
        'Review Luaran' => 'status_penilaian_luaran',
        'Review Antara' => 'status_penilaian_antara',
    ];

    $val = $data[$mapped[$agendaAktif]] ?? null;
