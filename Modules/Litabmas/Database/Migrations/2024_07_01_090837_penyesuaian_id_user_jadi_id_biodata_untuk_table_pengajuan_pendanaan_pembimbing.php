...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        // STEP 1: prepare
        // get current data pembimbing
        $data = DB::table('litabmas.pengajuan_pendanaan_pembimbing')->get();
        // tampung id_user
        $userIds = [];
        ...
        foreach ($data as $item) {
            ...
            DB::table('litabmas.pengajuan_pendanaan_pembimbing')->where('id_user', $item->id_user)->update(['id_biodata' => $biodataId]);
        }

        // STEP 4: after update
        // remove column id_user dari litabmas.pengajuan_pendanaan_pembimbing
        SevimaSchema::table('litabmas.pengajuan_pendanaan_pembimbing', function (SevimaBlueprint $table) {
            $table->dropColumn('id_user');
        });
        // update column id_biodata menjadi not nullable
        DB::statement('ALTER TABLE litabmas.pengajuan_pendanaan_pembimbing ALTER COLUMN id_biodata SET NOT NULL');
    ...
    public function down()
    {
        ...
        foreach ($data as $item) {
            ...
            DB::table('litabmas.pengajuan_pendanaan_pembimbing')->where('id_biodata', $item->id_biodata)->update(['id_user' => $userId]);
        }

        // STEP 4: after update
        // remove column id_biodata dari litabmas.pengajuan_pendanaan_pembimbing
        SevimaSchema::table('litabmas.pengajuan_pendanaan_pembimbing', function (SevimaBlueprint $table) {
            $table->dropColumn('id_biodata');
        });
        // update column id_user menjadi not nullable
        DB::statement('ALTER TABLE litabmas.pengajuan_pendanaan_pembimbing ALTER COLUMN id_user SET NOT NULL');
